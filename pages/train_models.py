...
def save_feature_importance(model, X_train, y_train, X_test, y_test, filename):
    ...
    plt.ylabel('Cecha')
    plt.tight_layout()
    plt.savefig(f"pictures/{filename} Feature Importance.png")
    plt.close()
...
def train_models(df):
    ...
    models = {
        ...
        'Regresja wielomianowa': make_pipeline(PolynomialFeatures(degree=2), StandardScaler(), LinearRegression()),
        'Drzewa decyzyjne': DecisionTreeRegressor(random_state=42),
        'Lasy losowe': RandomForestRegressor(n_estimators=85, random_state=42),
    ...
    models['K Najbliższych Sąsiadów KNN'] = best_knn

    for name, model in models.items():
        if name == 'K Najbliższych Sąsiadów KNN':  # Skalowanie dla KNN
        ...
        pd.DataFrame([metrics]).to_csv(f"metrics/{name} metrics.csv", index=False)
        ...
        # Zapis modeli
        joblib.dump(model, f"models/{name} model.sav")
