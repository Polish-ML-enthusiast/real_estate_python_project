...
def load_model_metrics(model_name):
    try:
        model = joblib.load(f"models/{model_name} model.sav")
        metrics = pd.read_csv(f"metrics/{model_name} metrics.csv")
        return model, metrics
    except FileNotFoundError:
        st.error(f"Nie znaleziono pliku dla modelu {model_name}. Sprawdź, czy plik istnieje.")
        return None, None
    except Exception as e:
        st.error(f"Wystąpił błąd przy ładowaniu danych modelu {model_name}: {e}")
        return None, None

def plot_feature_importance(model_name):
    plot_path = f"pictures/{model_name} Feature Importance.png"
...
def app():
    ...
    if 'navigate_to_results' in st.session_state and st.session_state['navigate_to_results']:
        ...
        for model_name in selected_models:
            model, metrics = load_model_metrics(model_name)
            if model:
                prediction = model.predict(input_df)[0]
                if model_name == 'K Najbliższych Sąsiadów KNN':
                    model_display_name = f"{model_name} - k: {model.n_neighbors}"
                else:
                ...
                feature_importance_plot = plot_feature_importance(model_name)
        ...
        for result in results_data:
            model_key = result['Model'].split(' - ')[0]
            plot_path = f"pictures/{model_key}.png"
            if os.path.exists(plot_path):
